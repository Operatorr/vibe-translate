...
> **Figure note:** The original article includes diagrams showing Codex driving the app with Chrome DevTools MCP to validate work, and a local observability stack that exposes logs, metrics, and traces to Codex for feedback-loop driven validation.
...
- **It’s hard to verify.** A single blob doesn’t lend itself to mechanical checks, such as coverage, freshness, ownership, and cross-links, so drift is inevitable.
...
> **Figure note:** The original article includes a diagram explaining “the limits of agent knowledge”: unseen knowledge from Google Docs, Slack messages, and tacit team knowledge must be encoded into the codebase as markdown for Codex to access it.
...
Pulling more of the system into a form the agent can inspect, validate, and modify directly increases leverage - not just for Codex, but for other agents, such as Aardvark, that are working on the codebase as well.
...
The diagram below shows the rule: within each business domain, such as App Settings, code can only depend “forward” through a fixed set of layers: Types → Config → Repo → Service → Runtime → UI. Cross-cutting concerns - auth, connectors, telemetry, feature flags - enter through a single explicit interface: Providers. Anything else is disallowed and enforced mechanically.

> **Figure note:** The original article includes a layered domain architecture diagram with explicit cross-cutting boundaries: Utils feed into Providers, Providers connect to Service and App Wiring + UI, and a business logic domain contains Types, Config, Repo, Service, Runtime, and UI.
